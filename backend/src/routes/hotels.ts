...
router.post('/:hotelId/bookings/payment-intent', verifyToken,
  async (req: Request, res: Response) => {
    ...
    if (!paymentIntent.client_secret) {
      res.status(500).json({ message: 'Err creating payment intent' })
    }
...
router.post('/:hotelId/bookings', verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    console.log(err)
    res.status(500).json({ message: 'Err creating booking' })
    return
  }
})
...
router.get('/search', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: 'Err: [backend/src/routes/hotel.ts/router.get(/search)]' })
    return
  }
})
...
router.get('/:id',
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log(err)
      res.status(500).json({ message: 'Err fetching hotel' })
      return
    }
  }
)
...
const constructSearchQuery = (queryParams: any) => {
  ...
  //   const starRating = parseInt(queryParams.stars)
