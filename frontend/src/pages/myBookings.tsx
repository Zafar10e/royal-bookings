import * as apiClient from '../api-client'
import { useQuery } from '@tanstack/react-query'

const MyBookings = () => {
 const { data: hotels } = useQuery({
  queryKey: ['fetchMyBookings'],
  queryFn: apiClient.fetchMyBookings
 })

 if (!hotels || hotels.length === 0) {
  return <span>No bookings found</span>
 }

 return (
  <div className='space-y-5'>
   <h1 className='text-2xl font-semibold'>My Bookings</h1>
   {hotels.map(hotel => (
    <div className='grid grid-cols-1 md:grid-cols-[1.5fr_2.5fr] border border-gray-300 rounded-lg p-5 gap-5'>
     <div className='md:w-full md:h-[250px]'>
      <img
       src={hotel.imageUrls[0]}
       className='h-[300px] w-full md:w-full md:h-full object-cover object-center'
      />
     </div>
     <div className='flex flex-col gap-4 overflow-y-auto max-h-[300px]'>
      <div className='text-2xl font-semibold'>
       {hotel.name}
       <div className='text-xs font-normal'>
        {hotel.city}, {hotel.country}
       </div>
      </div>
      {hotel.bookings.map(booking => (
       <div>
        <div>
         <span className='font-semibold mr-2'>Dates: </span>
         <span>
          {new Date(booking.checkIn).toDateString()} -
          {new Date(booking.checkOut).toDateString()}
         </span>
        </div>
        <div>
         <span className='font-semibold mr-2'>Guests: </span>
         <span>
          {booking.adultCount} adults,
          {booking.childCount} childs
         </span>
        </div>
       </div>
      ))}
     </div>
    </div>
   ))}
  </div>
 )

}

export default MyBookings