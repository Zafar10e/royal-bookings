 ...
 return (
  <div className="flex flex-col min-h-screen">
   ...
   <form onSubmit={onSubmit} className="container mx-auto flex flex-col flex-1 gap-3">
    <h2 className="text-3xl font-bold text-blue-900 mt-3 py-2 md:py-5 px-10">Sign-in</h2>

    <div className="flex flex-col gap-4 px-10">
     <label className="flex flex-col font-semibold">
      Email:
      <input type="email" className={`border-2 ${errors.email ? 'border-red-500' : 'border-orange-400'} p-2 rounded font-normal focus:outline-none`} {...register('email', { required: '*Email is required!' })} />
      {errors.email && (
     ...
     <label className="flex flex-col font-semibold">
      Password:
      <input type="password" className={`border-2 ${errors.password ? 'border-red-500' : 'border-orange-400'} p-2 rounded font-normal focus:outline-none`} {...register('password', { required: '*Password is required!' })} />
      {errors.password && (
     ...
     <span className="flex flex-col sm:flex-row justify-between py-10">
      ...
      <button type="submit" className="bg-blue-800 px-10 py-2 rounded text-white text-xl font-semibold hover:bg-blue-700 hover:underline mt-5">Login</button>
     </span>
    </div>
   </form>
   <Footer />
